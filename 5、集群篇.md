...
填入token名称【gmall-analyze】，点击Generate创建token
...
3、登录ws-manager创建企业空间
	1）创建企业空间gmall-workspace
	2）指派管理员ws-admin
	
4、登录ws-admin邀请企业成员
	1）project-admin：可以创建项目	 role：gmall-workspace-self-provisioner
	2）project-regular：只能查看项目 role：gmall-workspace-viewer
	
5、登录project-admin创建项目【测试】
	测试1）创建资源型项目gmall，并邀请project-regular为该项目的开发人员
	测试2）创建DevOps项目
...
```
1、登录ws-manager创建企业空间wordpress，指派ws-admin作为该企业空间的管理员【gmall-workspace-admin】

2、登录ws-admin邀请人员进入该企业空间，指定project-admin为项目管理者【可以创建项目gmall-workspace-self-provisioner】，project-regular【不可以创建项目gmall-workspace-viewer】
...
binlog-do-db=gmall_wms
binlog-do-db=gmall_admin
...
一、创建主机
1、创建配置
	1）gmall-mysql-master-cnf，别名：master配置
	2）配置设置
	key:my.cnf
	value:
[client]
...
4、创建成功，查看服务mysql-master
	1）查看DNS: mysql-master.gmall
	2）随便进入一个容器的终端： ping mysql-master.gmall可以ping通

二、创建从机
1、创建pvc：gmall-mysql-slaver-pvc
	读写：10Gi
...
5、设置主库连接，这里的master-host填写之前的DNS
	change master to master_host='mysql-master.gmall',master_user='backup',master_password='123456',master_log_file='mysql-bin.000001',master_log_pos=0,master_port=3306;
...
4、验证
	1）使用admin登录kubesphere：192.168.56.100:30880/login
		admin/P@88w0rd
	2）打开控制台：curl http://elasticsearch.gmall:9200【服务的DNS，查看是否有打印】
```
...
1、创建一个无状态服务
	...
	镜像：kibana:7.4.2
	环境变量：
		ELASTICSEARCH_HOSTS	http://elasticsearch.gmall:9200【DNS地址】
	2）选中外网访问
		NodePort
...
重点：这里无状态服务开启对外服务是NodePod 随机端口的方式对外暴露，所以在其他pod节点上使用域名是访问不到的，例如在zipkin容器中ping nacos.gmall ：失败
所以后面部署的应用服务的配置spring.cloud.nacos.discovery.server-addr不能直接使用nacos.gmall
...
2、测试
进入zipkin终端
	ping nacos-service.gmall

3、修改各项目配置文件中
spring.cloud.nacos.discovery.server-addr=nacos-service.gmall:8848
...
1、创建无状态服务【数据适用es存储】
	...
	ES_HOSTS		elasticsearch.gmall:9200
	3）设置外网访问
	NodePort
...
2、测试
进入zipkin终端
	ping sentinel-service.gmall

3、修改各项目配置文件中
spring.cloud.sentinel.transport.dashboard=sentinel-service.gmall:8333
...
spring.datasource.url=jdbc:mysql://mysql-master.gmall:3306/gmall_sms?useUnicode=true&characterEncoding=UTF-8&useSSL=false&serverTimezone=Asia/Shanghai
spring.redis.host=redis.gmall【6379】
spring.rabbitmq.host=rabbitmq-management.gmall【5672】
elasticsearch.gmall【9200】
...
名称：项目名-deploy.yaml
例：gmall-auth-server-deploy.yaml
	# namespace名称空间是项目
	...
	# port: 8080，通过域名  gmall-auth-server.gmall:8080 内部访问的，这里是NodePort对外暴露，以节点的方式访问192.168.56.100:20001
...
3、为每一个微服务创建一份 xx-deploy.yaml
	修改三个部分
	1、修改成各自项目名name: gmall-auth-server
	2、app选择器修改成name: gmall-auth-server
	3、nodePort修改为不同的端口
```
...
```
1、未指定编译后代码位置，在gmall的 pom.xml 中指定properties和build
...
```
gmall-auth-server-deploy.xml部署文件里面，镜像应该选择最新镜像而不是发布版本的镜像
...
1、复制Jenkinsfile文件到项目gmall最外层，默认构建 网关模块
...
原因：
	...
	例如gmall-cart发布v1.0成功，但是因为标签没有加项目名字，导致发布gmall-product的v1.0版本没哟项目名作为区分，发布版本失败。
	修改：
    stage('发布版本'){
      when{
        expression{
...
修改gmall-product和gmall-seckill的MyRedissonConfig.java配置类：
	自动获取配置文件中redis的host
@Configuration
...
-m：提交的一些信息
容器id
docker commit -a "wanzenghui" -m "nginx" [CONTAINER NAME] wanzenghui/gmall-nginx:v1.0

2、登录docker hub
docker login -u wanzenghui

3、推送
docker push wanzenghui/gmall-nginx:v1.0
...
5、重新构建镜像
docker commit -a "wanzenghui" -m "nginx-update-upstream" 97b83151128b gmall-nginx:v1.2
这里演示，名字打错了，要改成wanzenghui/gmall-nginx:v1.2
	查看镜像docker [imagesID]
	docker tag gmall-nginx:v1.2 wanzenghui/gmall-nginx:v1.0
	
6、3、推送
docker push wanzenghui/gmall-nginx:v1.2

```

本地镜像：wanzenghui/gmall-nginx
...
3、使用阿里云镜像仓库 上传nginx镜像
	...
	2）拉取镜像
	docker pull registry.cn-shanghai.aliyuncs.com/wanzenghui/gmall-nginx:v1.0
	3）推送
	docker commit -a "wanzenghui" -m "nginx" 97b83151128b wanzenghui/gmall-nginx:v1.0
	docker images查看上面的image ID
	docker tag [ImageId] registry.cn-shanghai.aliyuncs.com/wanzenghui/gmall-nginx:v1.0
	docker push registry.cn-shanghai.aliyuncs.com/wanzenghui/gmall-nginx:v1.0
	4）上传成功
...
2、创建无状态服务
	名称：gmall-nginx
	容器镜像：找到刚刚配置的镜像仓库
	输入镜像名字：wanzenghui/gmall-nginx:v1.0
	暴露外网访问NodePort
	创建成功，查看port 30498
...
1、使用project-admin管理员账户登录kubesphere
	1）进入项目gmall=》项目设置=》高级设置=》设置网关=》访问方式：loadBalancer
	2）删除默认的两个注解qingcloud

2、创建应用路由
	1）进入项目gmall=》应用负载=》应用路由=》创建路由
	2）名称：gmall-com
	3）添加路由规则
		指定域名：
			域名：gmall.com
...
```
gmall.com在本地配置成了192.168.56.101
然后访问gmall.com/static/search/image/bg-attr.png出现异常
原因查询：
	进入gmall-nginx，查看容器日志
...
ENTRYPOINT nginx -g "daemon off;"
	...
	docker build -t gmall-nginx:v1.0 -f Dockerfile .
	...
	6、删除gmall-nginx服务+部署【也可以直接在原服务上修改】
		1）重新部署无状态服务，容器镜像选择阿里云，镜像选择wanzenghui/gmall-nginx:v1.0
		使用默认端口
		勾选外网访问+NodePort
...
192.168.56.101 gmall.com
192.168.g6.101 search.gmall.com
192.168.56.101 item.gmall.com
192.168.56.101 auth.gmall.com
192.168.56.101 cart.gmall.com
192.168.56.101 order.gmall.com
192.168.56.101 member.gmall.com
192.168.56.101 seckill.gmall.com
...
3、构建镜像
	...
	docker build -t gmall-admin-vue-app:v1.0 -f Dockerfile .
...
5、k8s部署，无状态服务
	1）创建一个无状态服务
	名称：gmall-admin-vue-app
	添加容器镜像：wanzenghui/gmall-admin-vue-app:v1.0
	使用默认端口
	选择外网访问，访问方式NodePort
	...
	3）使用ingress转发
	添加路由规则，admin.gmall.com转发给gmall-admin-vue-app服务，80端口
	本地使用admin.gmall.com访问
