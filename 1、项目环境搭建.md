...
3、.gitignore文件：忽视要提交的文件【在gmall统一配置】
```
...
```
1、创建5个微服务
商品服务(product)：gmall_pms
库存服务(ware)：gmall_wms
订单服务(order)：gmall_oms
优惠券服务(coupon)：gmall_sms
会员服务(member)：gmall_ums

2、选中gmall创建module
	com.atguigu.gmall：group
	gmall-product：Artifact
	Description：商品服务
	package：改一下，去掉gmall前缀，如下图
...
3、将renren-fast项目拷贝到gmall项目文件夹中，在pom加入子模块
	<modules>
		<module>renren-fast</module>
	</modules>

4、创建数据库，建表语句在renren-fast->db->mysql.sql中
	创建数据库gmall_admin，根据建表语句创建表

5、修改renren-fast的配置文件 
application-dev.yml
	driver-class-name: com.mysql.cj.jdbc.Driver
    url: jdbc:mysql://192.168.56.10:3306/gmall_admin?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai
    username: root
    password: root
...
2、spring管理数据源
3、mybatis-plus配置
	1）扫描注解：@MapperScan("com.atguigu.gmall.product.dao")
...
1、整合mybatis-plus
	...
	2)代码层面整合，参照文档：https://mp.baomidou.com/guide/config.html
		...
		2、配置mybatis-plus
			1)在product模块的Application类上@MapperScan("com.atguigu.gmall.product.dao")
...
3、Application添加@EnableFeignClients(basePackages="com.atguigu.gmall.member.feign")
```
...
2、使用member远程调用coupon作为demo
    ...
    【远程拉取gmall-coupon服务，发送请求/coupon/coupon/member/list】
...
3、开启远程调用功能，在调用方的Application添加注解@EnableFeignClients(basePackages="com.atguigu.gmall.member.feign")
...
1、命名空间：用于服务间隔离
默认：public
	1）、在Nacos页面新建命名空间，然后新增gmall-coupon.properties属性文件并指定为新创建的命名空间，记录下该命名空间的id：37d3931b-aac3-4254-894b-3f6d8b35f5fa
...
bootstrap.properties配置文件的作用：
	...
	6）默认情况，如果不指定3、4会加载public命名空间下DEFAULT_GROUP下Data Id=gmall-coupon的文件
...
会根据请求头的host值来反向代理，如果host=gmall.com
则下面这个块会反向代理访问gmall.com:80的请求，因为host带的是gmall.com
访问：gmall.com，默认转发到	/usr/share/nginx/html/index.html
server {
    listen       80;
...
配置gmall.conf
	server {
        ...
        location / {
            proxy_pass http://gmall; # 转发到上游服务器，对应nginx.conf的gmall
        }
    }
...
server {
    listen       80;
    server_name  gmall.com *.gmall.com iua4i6.natappfree.cc;
